...
public class Grave 
{
    ...
    public string PlayerColor => PlayerIndex switch
        {
    ...
    {
        Texture = _engine.Content.Load<Texture2D>($"Mole/{PlayerColor}_grave");
    }
    ...
    public Renderable? GetSpriteProjection(Mole player)
    {
            ...
            && normalizedDistance > 0.5f)
        {
            ...
            var projectionWidth = projection * ratio;
            var projectionHeight = projection;
            var halfProjectionWidth = (int)projectionWidth / 2;
            var offsetY = projectionHeight * OffsetY;
            var projectionX = screenX - halfProjectionWidth;
            var projectionY = Settings.ScreenHalfHeight - ((int)projectionHeight / 2) + (int)offsetY;
